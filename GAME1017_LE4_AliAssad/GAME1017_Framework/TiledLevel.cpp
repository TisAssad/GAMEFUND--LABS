	...
	const char* tileData, const char* levelData, const char* tileKey) :m_rows(rows), m_cols(cols), m_tileKey(tileKey)
{
	...
	{
		char key;
		m_level.resize(m_rows); //must resize to use subscripts
...
void TiledLevel::Render()
{
	for (unsigned row = 0; row < m_rows; row++)
	{
		for (unsigned col = 0; col < m_cols; col++)
		{
			SDL_RenderCopyF(REMA::GetRenderer(), TEMA::GetTexture(m_tileKey), m_level[row][col]->GetSrc(), m_level[row][col]->GetDst());
		}
	}
}
